...
class ScreenUtils {
  ...
  double _screenPixelRatio = 0.0;  // 设备分辨率
  double _statusBarHeight = 0.0;   // 状态栏高度
  double _bottomBarHeight = 0.0;   // 底部bar高度
  double _appBarHeight = 0.0;    // 状态栏高度
  /// MediaQueryData  获取当前设备的信息
  ...
  _init() {
    MediaQueryData mediaDara = MediaQueryData.fromWindow(ui.window);
    if (_mediaQueryData != mediaDara) {
      _mediaQueryData = mediaDara;
      _screenWidth = mediaDara.size.width;
      _screenHeight = mediaDara.size.height;
      _screenPixelRatio = mediaDara.devicePixelRatio;  /// 设备分辨率
      _statusBarHeight = mediaDara.padding.top;
      _bottomBarHeight = mediaDara.padding.bottom;
  ...
  /// 屏幕像素
  double get screenPixelRatio => _screenPixelRatio;
  ...
  double getSp(double fontSize) {
    if (_screenPixelRatio == 0.0) return fontSize;
